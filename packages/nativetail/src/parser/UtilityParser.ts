...
export default class UtilityParser {
	...
	private parseUtility(): StyleIR | null {
		...
		if (this.consumePeeked(`translate-y-`)) {
			...
			return {
				...
				complete: (style) => {
					...
					const styleValue = {
						translateY: translateY[0] * (this.isNegative ? -1 : 1),
					};
					if (style.transform && Array.isArray(style.transform)) {
						style.transform = [
							...style.transform.filter(
								(t) => !Object.keys(t).includes("translateY")
							),
							styleValue,
						];
					} else {
						style.transform = [styleValue];
					}
				},
			};
		}
			...
			return {
				...
				complete: (style) => {
					...
					const translateValue =
						this.rest == "full"
							? `${this.isNegative ? "-" : ""}100%`
							: translateX[0] * (this.isNegative ? -1 : 1);
					const styleValue = {
					...
					if (style.transform && Array.isArray(style.transform)) {
						style.transform = [
							...style.transform.filter(
								(t) => !Object.keys(t).includes("translateX")
							),
							styleValue,
						];
					} else {
		...
		if (this.consumePeeked(`rotate-`)) {
			...
			return {
				...
				complete: (style) => {
					if (!rotate) return;
					const styleValue = {
						rotate: `${rotate[0] * (this.isNegative ? -1 : 1)}deg`,
					};
					if (style.transform && Array.isArray(style.transform)) {
						style.transform = [
							...style.transform.filter(
								(t) => !Object.keys(t).includes("rotate")
							),
							styleValue,
						];
					} else {
				...
				return {
					...
					complete: (style) => {
						...
						if (style.transform && Array.isArray(style.transform)) {
							style.transform = [
								...style.transform.filter(
									(t) => !Object.keys(t).includes("scale")
								),
								styleValue,
							];
						} else {
		...
		if (this.consumePeeked(`select-none`)) {
			return h.complete({
				style: {
					userSelect: `none`,
				},
		...
		if (handledUtilities.some((u) => this.consumePeeked(`${u}`))) {
			return null;
		}
